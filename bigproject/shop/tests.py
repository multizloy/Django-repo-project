...
from .models import Product, Category, Product_Proxy
...
class ProductDetailViewTest(TestCase):
    def test_get_product_by_slug(self):
        ...
        product = Product.objects.create(
            name="Product-1", category=category, slug="product-1", image=uploaded
        )
        # Make a request to the product detail view with the product's slug
        response = self.client.get(
            reverse("shop:products-detail", kwargs={"slug": "product-1"})
        )
...
class CategoryListViewTest(TestCase):
    def setUp(self):
        ...
        self.product = Product_Proxy.objects.create(
            name="Test Product",
            slug="test-product",
